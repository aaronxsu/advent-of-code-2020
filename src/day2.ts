...
const dayTwoInput = `1-8 n: dpwpmhknmnlglhjtrbpx
...
const countValidPasswordsPartOne = (input: string): number => {
  let count = 0;
  input.split("\n").forEach((line) => {
    const sections = line.split(" ").filter((s) => s !== "");
    if (sections.length) {
      const [min, max] = sections[0].split("-");
      const minOccurance = parseInt(min);
      const maxOccurance = parseInt(max);
      const targetLetter = sections[1].replace(":", "");
      const password = sections[2];
      const occurances = password.split("").reduce((acc, letter) => {
        if (acc[letter]) {
      ...
      if (
        occurances[targetLetter] &&
        occurances[targetLetter] >= minOccurance &&
        occurances[targetLetter] <= maxOccurance
      ) {
        count = count + 1;
      }
    }
...
console.info(`Part 1: ${countValidPasswordsPartOne(dayTwoInput)}`);
// Part 1: 445
...
const countValidPasswordsPartTwo = (input: string): number => {
  let count = 0;
  input.split("\n").forEach((line) => {
    const sections = line.split(" ").filter((s) => s !== "");
    if (sections.length) {
      const [min, max] = sections[0].split("-");
      const minIdx = parseInt(min) - 1;
      const maxIdx = parseInt(max) - 1;
      const targetLetter = sections[1].replace(":", "");
      const password = sections[2];
      if (
        (password[minIdx] === targetLetter &&
          password[maxIdx] !== targetLetter) ||
        (password[minIdx] !== targetLetter && password[maxIdx] === targetLetter)
      ) {
        count = count + 1;
      }
    }
...
console.info(`Part 2: ${countValidPasswordsPartTwo(dayTwoInput)}`);
// Part 2: 491
